...
# базовая фитнес функция
creator.create("BaseFitness", base.Fitness, weights=(1.0,))
# на основе фитнес функции создаются
# описание в ga_exp
# speed, smin и тд это атрибуты (как и фитнесс)
...
class PSOAlg:
    ...
    def particle_generation(self):
        # создает учатистника с помощью метода дженерейт на основе ранее созданного класса particle

        # на основе созданной вначале Particle с помощью метода генерейт
        particle = tools.initIterate(creator.Particle, self.generate)
        # случайная скорость задайтся частице
        # все эти атрибуты были созданы при инициализации в creator.create()
    ...
    def __init__(self, pop_size, iterations, dimension, function):
        ...
        self.c2 = 0.7
        self.w = 1.0
        # границы, используемые при инициализации участника
        self.pmin = -5.0
        self.pmax = 5.0
        # скорость минимальная и максимальная
        self.smin = -2.0
        self.smax = 2.0


        self.toolbox = base.Toolbox()
        # создать новоую частицу, вроде
